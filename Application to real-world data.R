...
data_CA <- data_CA[(1:287),]　#change values of row number
data_stan <- data_CA %>% select(Date, PMMoV_Los,IAV_Los) #areas
...
sp <- 241 # row number of maximum value of Rt
ww <- data_stan %>% select(Date, median) %>% mutate(median = 10^(median))
ww <- ww[sp:int_p,]
Ws_0 <- sum(ww$median)
Ws_1 <- sum(ww$median) - ww[int_p-sp,2] 
Rt_0 <- WW_Rt[int_p-8, 3] 
Rt_1 <- WW_Rt[int_p-9, 3]
W0 <- (Ws_1*Rt_0-Ws_0*Rt_1)/(Rt_0-Rt_1)
Ws_ini <- W0 - Ws_1
...
write.csv(x = data_result_7, file = "/home/u31/hirokiando/20240629_Prediction_1222_South_IAV.csv")
