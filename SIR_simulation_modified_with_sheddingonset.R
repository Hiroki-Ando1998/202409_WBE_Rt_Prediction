...
prob <- NULL
for(i in 0:7){
  probability <- gamma_probability(i, i + 1, shape_1, rate_1)
  prob <- c(prob, probability) 
}
gamma_prob_shed <- data.frame(prob)
...
# パラメータ
parameters <- c(beta = 0.50, gamma = 0.25) #0.50, 0.375, 0.875 
...
#shedding start #shedding means the number of people who start shedding on the day
shedding <- numeric()
run_4 <- nrow(data_sim_3)
run_5 <- nrow(gamma_prob_shed)
for(i in (run_5+1):run_4){
  n_shedding_2 <-0
  for(m in 1:run_5){
    n_shedding_1 <- data_sim_3$incidence_2[i-m]*gamma_prob_shed$prob[m]
...
data_xx_4 <- cbind(data_sim_3, data_shedding)



#Convert wastewater concentration
data_shedding <- read.csv("Assumed_shedding.csv")
wastewater <- numeric()
run_6 <- nrow(data_xx_4)-1
for(i in 26:run_6){
  n_wastewater_2 <-0
  for(m in 1:25){
    n_wastewater_1 <- data_xx_4$shedding[i-m]*data_shedding$super_rapid[m] #rapid, middle, prolong, super_rapid
...
WBE <- log10(data_stan_removed$wastewater)
...
mcmc_eff_cli_incidence <- stan(
  file = "20240515_clinical_Re_2_7day.stan", #5(rapid), 11(middle), 7(prolong)
  data = data_list_eff_2,
  seed = 1,
  chain = 1,
  iter = 10000,
...
colnames(result_eff_cli_21_incidence) <- c("low_incidenc_5", "med_incidenc_5", "upr_incidenc_5")
...
#data_eff_xx_2 <- cbind(data_date_xx, data_cli_xx, data_cli_xx_2, data_result_1, data_result_2, data_result_3)


#まとめる for shedding onset with 2 day
b <- nrow(result_eff_cli_1_incidence)-shape_1/rate_1
data_cli_xx <- result_eff_cli_1_incidence[12:b,]
data_cli_xx_2 <- result_eff_cli_21_incidence[12:b,]
data_date_xx <- data_sim_final_2[12:b,] %>% select(time)
a <- nrow(data_result_1)
a_1 <- 1 + shape_1/rate_1
data_result_1_a <- data_result_1[a_1:a,]
data_result_2_a <- data_result_2[a_1:a,]
data_result_3_a <- data_result_3[a_1:a,]

data_eff_xx_2 <- cbind(data_date_xx, data_cli_xx, data_cli_xx_2, data_result_1_a, data_result_2_a, data_result_3_a)
...
print(average_error_1_b)
 ...
 #three day sampling
  ...
  data_re_21_incidence <- data_re_subset %>% select("time", "low_incidenc_21", "med_incidenc_21", "upr_incidenc_21")
  data_name_re_21_inc <- data.frame(c = "21day_cli_incidence")
  data_re_21_incidence <- cbind(data_re_21_incidence, data_name_re_21_inc)
  colnames(data_re_21_incidence) <- c("time", "min", "median", "max", "Re_type")  
  ...
  data_sub_re_file <- rbind(data_re_1_incidence, data_re_21_incidence, data_re_ww_1, data_re_ww_3, data_re_ww_7)
